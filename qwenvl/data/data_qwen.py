                ...
                if visual_tag in content:
                    ...
                    for i in range(len(parts) - 1):
                        ...
                        if visual_replicate_index >= len(grid_thw):
                            raise ValueError(
                                f"Mismatch between visual tags ({len(parts)-1}) and grid_thw entries ({len(grid_thw)})"
                            )
                        replacement = (
                            "<|vision_start|>"
                            + f"<|{visual_type}_pad|>"
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0] or "images" in sources[0]:
            ...
            if len(grid_thw) != num_visual_tags:
                error_msg = (
                    f"Sample {i} validation failed: {num_visual_tags} visual tags vs {len(grid_thw)} grid_thw entries\n"
                    f"Conversation content: {conversation}\n"
